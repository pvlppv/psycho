...
class Message_EN_Create(BaseModel):
    message_text: str
    lobby_name: str

    @validator("message_text")
    def message_text_validator(cls, v):
        if len(v) < 100:
        ...
        email_regex = re.compile(
            r"(?i)\b(https?|www|[a-zа-я0-9]+\.(com|ru|net|io|me|org|edu|info|biz|co|gov|space|website|site|su|store|pro|online|xyz|fun|tech|shop))\b"
        )
        ...
        profanity_regex = re.compile(
            r"(?i)\b(cunt|twat|whore|slut|nigger|nigga|faggot|homo|retard|autistic|virgin|incel|simp|chink|spic|kike|wetback|fag|dyke|queer|whore|motherfucker)\b"
        )
...
class Message_RU_Create(BaseModel):
    message_text: str
    lobby_name: str
    ...
    def message_text_validator(cls, v):
        ...
        profanity_regex = re.compile(
            r"(?i)\b(наркотик|негр|нигер|ниггер|нигга|nigger|nigga|faggot|пидор|пидорас|педик|гомик|петух|чурка|хач|хохол|хиджаб|фашист|нацист|бандеровец|арийский|жид|семит|скинхед|расист|кафр|шварц|мулла|даун|аутист|девственник|симп|инцел|cunt|куколд)\b"
        )
        if any(
            profanity_regex.search(word)
            or profanity_regex.search(morph.parse(word)[0][2])
            for word in v.split()
        ):
